/**
 * A simple server that can serve up the B2G Gaia UI.
 *
 * Runs the dev code: server.js docRoot=path/to/gaia/apps
 *
 * Valid options are:
 *
...
var http = require('http'),
    ...
    port = options.port || 8424,
    expectedHost = 'gaiamobile.org',
...
function handleRequest(req, res) {
    ...
    if (req.method === 'GET') {
        ...
        if (hostParts.length > 2) {
            urlPath = path.resolve(docRoot, subdomain, urlPath);
        } else if (host.indexOf(expectedHost) === 0) {
            //A gaiamobile.org load, which is from a directory up from the
            //gaia/apps directory.
...
//Do some validation. Only start up the server if the docRoot looks like
//gaia. In particular, there should be a "homescreen" directory
if (!path.existsSync(path.join(docRoot, 'homescreen'))) {
    throw new Error('docRoot does not seem to be the Gaia apps directory, ' +
                    'no homescreen subdirectory.');
}
